    ...
    <div class="item col-xs-12 col-md-12 panel-body panel panel-default">
      <div class="table-responsive">
        ...
        >
          ...
          <tbody>
            <tr v-for="(privacy, index) in privacys" :key="privacy.id">
              <td>{{ index + 1 }}</td>
              <td>
                <img
                  :src="'/images/' + privacy.image"
                  class="img-responsive"
                  height="35"
                  width="70"
                />
              ...
              <td>
                <div v-html="privacy.details"></div>
              </td>
              <td>
                <router-link
                  :to="{ name: 'editprivacy', params: { id: privacy.id } }"
                  class="btn btn-primary"
                  ><i class="fa fa-edit"></i>Edit</router-link
                >
                <button
                  class="btn btn-danger"
                  @click.prevent="deletePost(privacy.id)"
                >
  ...
  methods: {
    add() {
      this.form
        .post("api/privacys", {
          method: "post",
          body: JSON.stringify(this.privacy),
          headers: {
    ...
    viewPrivacy() {
      fetch("api/privacys")
        .then((res) => res.json())
        .then((res) => {
          this.privacys = res.data;
        })
        .catch((err) => console.log(err));
    },

    deletePost(id) {
      fetch(`api/privacys/${id}`, {
        method: "delete",
        body: JSON.stringify(this.privacy),
        headers: {
          "content-type": "application/json",
